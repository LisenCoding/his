...
//        queryFineTuningJobRequest.setAfter("1");
    ...
    public static ImageApiResponse testCreateImage(String src) {
        CreateImageRequest createImageRequest = new CreateImageRequest();
        createImageRequest.setModel(Constants.ModelCogView);
      createImageRequest.setPrompt(src);
   ...
   return imageApiResponse;
     ...
     * sse调用
     */
    public static ModelApiResponse testSseInvoke(String text) {
        List<ChatMessage> messages = new ArrayList<>();
...
//         ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), "你能帮我查询2024年1月1日从北京南站到上海的火车票吗？");
   ...
   return sseModelApiResp;
     ...
     * 同步调用
     */
    public static ModelApiResponse testInvoke(String text) {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), text);
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
